...
def plot_loss_lr(D_loss, G_loss, L1_loss, L2_loss, G_LR_plot, D_LR_plot, epoch=None):
...
def save_gif(dir_to_imgs, L1_loss, L2_loss):
    ...
    filenames = [filename for filename in os.listdir(dir_to_imgs) if filename.startswith('y')]
    for filename, l1, l2 in zip(filenames, L1_loss, L2_loss):
        ...
        draw = ImageDraw.Draw(img).text((0, 0),f"epoch: {img_number+1}/{config.NUM_EPOCHS}",(255,255,255))
        draw = ImageDraw.Draw(img).text((0, 10),f"L1: {l1:.3f}")
        draw = ImageDraw.Draw(img).text((0, 20),f"L2: {l2:.3f}")
        generated_images.append(img)
    generated_images[0].save(dir_to_imgs+"/out.gif", save_all=True, append_images=generated_images[1:], duration=300, loop=1)
